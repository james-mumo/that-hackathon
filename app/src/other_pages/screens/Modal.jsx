...
function Modal({ isOpen, onClose, bus }) {
  ...
  const [reserveSpecialSeat, setReserveSpecialSeat] = useState("no");
  ...
  const viewBooking = () => {
    ...
    axios
      .post(book_endpoint, {
        ...
        busInfo: bus,
        isPaid: false,
        reserveSpecialSeat: reserveSpecialSeat, // Include the special seat reservation
      })
      .then((response) => {
        // Handle success
        ...
        toast.success("Booking successful! Thank you for choosing us!");
  ...
  return (
    <>
      {isOpen && (
        <div className="modal absolute inset-0 flex flex-col justify-center items-center bg-gray-900 bg-opacity-80 px-2">
            ...
            <div className="bg-gray-800 border-l border-r border-gray-950 py-4 px-2 flex shadow-lg">
              ...
              <div className="flex flex-col w-full  border border-teal-400 rounded-md border-opacity-40">
                ...
                <div className="flex px-3">
                  ...
                  <span className="text-[12px] gap-2 flex">
                    {bus.timeToCurrentLocation} mins away
                  </span>
                </div>
                <div className="flex px-3 mt-2">
                  <span className="px-0 ml-0 flex flex-1 flex-col gap-0 text-[10px] font-medium text-yellow-400">
                    <span className="text-[13px] font-medium text-emerald-400 mb-2 rounded-md">
                      <span className="bg-teal-800 px-1 py-2 text-white">
                        Seats Available: {bus.capacity - 1}
                      </span>
                    </span>
                    <span className="flex italic">{bus.sacco}</span>
                    <span className="text-sm"> {bus.numberPlate}</span>
                  </span>

                  {/* Checkbox for reserving a special seat */}
                  <div className="flex flex-col mt-2">
                    <label className="flex items-center text-sm text-emerald-400">
                      <input
                        type="checkbox"
                        className="mr-2 text-teal-500 focus:ring-teal-500"
                        value={reserveSpecialSeat}
                        onChange={(e) =>
                          setReserveSpecialSeat(e.target.checked ? "yes" : "no")
                        }
                      />
                      Reserve Special Seat
                    </label>
                  </div>
                </div>
              </div>
            ...
            {showQRCode && (
              <div className="p-2 flex-col align-middle items-center flex justify-center bg-gray-800 border-r border-l border-gray-950">
                ...
                <span className="text-[13px] w-full text-center ">
                  Scan this to reserve a seat and show the message generated
                  when boarding.
                </span>
                <span
                  onClick={viewBooking}
